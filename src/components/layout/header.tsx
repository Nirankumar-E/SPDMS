      ...
      <div className="bg-primary text-primary-foreground">
        <div className="container mx-auto px-4 flex flex-col sm:flex-row justify-between items-center text-center sm:text-left py-1 gap-1 sm:gap-0">
          <div className="text-xs">
            <span>{headerI18n.helpline}: </span>
            <a href="tel:1967" className="hover:underline">1967</a> | <a href="tel:18004255901" className="hover:underline">1800-425-5901</a>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2">
                <Button variant="ghost" size="sm" className={`h-7 px-2 text-xs hover:bg-primary/80 ${language === 'TA' ? 'font-bold underline' : ''}`} onClick={() => setLanguage('TA')}>தமிழ்</Button>
                <span className="text-sm">|</span>
                <Button variant="ghost" size="sm" className={`h-7 px-2 text-xs hover:bg-primary/80 ${language === 'EN' ? 'font-bold underline' : ''}`} onClick={() => setLanguage('EN')}>English</Button>
            </div>
             ...
             {loading ? (
              ...
              <div className="flex items-center gap-2">
                <Button variant="ghost" size="sm" className="h-7 px-2 text-xs hover:bg-primary/80 flex items-center gap-1" asChild>
                  <Link href="/dashboard">
                    <User className="h-4 w-4" /> {headerI18n.myProfile}
                  </Link>
                </Button>
                <span className="text-sm">|</span>
                <Button variant="ghost" size="sm" onClick={handleLogout} className="h-7 px-2 text-xs hover:bg-primary/80 flex items-center gap-1">
                  <LogOut className="h-4 w-4" /> {headerI18n.logout}
                </Button>
              </div>
            ) : (
              <Button variant="ghost" size="sm" className="h-7 px-2 text-xs hover:bg-primary/80 flex items-center gap-1" asChild>
                <Link href="/login">
                  <LogIn className="h-4 w-4" /> {headerI18n.citizenLogin}
                </Link>
              </Button>
            )}
          </div>
